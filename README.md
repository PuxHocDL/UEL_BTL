...
1. **Phân phối giá phòng (Price Distribution):**
   * Phần lớn listing tập trung ở vùng giá thấp - trung bình.
   * Có đuôi phải dài do một số listing cao cấp, đây là điểm cần chú ý khi chọn mô hình.
![Price Distribution](./price_distribution.png)

2. **Ma trận tương quan (Correlation Matrix):**
   * `price` có tương quan thuận với nhóm biến về sức chứa/phòng và chi phí dịch vụ.
   * Các biến `accommodates`, `bedrooms`, `beds` cũng tương quan lẫn nhau, gợi ý rủi ro đa cộng tuyến cho mô hình tuyến tính.
